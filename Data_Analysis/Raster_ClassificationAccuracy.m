...
for press_no = press_range % iterate through touch events
    ...
    A_rand_ID = 0; %find starting position of this frame
    ...
    y = D(:,frame_start:frame_end,press_no); %get the measurements with this starting position 
    ...
    for sample_level = 3:100 %iterate through all measurements levels to reconstruct this press
        vote_total = (frame_end-frame_start+1) / vote_period;
        recon_total = frame_end-frame_start+1; %determine how many reconstructions needed to reconstruct from frame_start to frame_end
        ...
        for recon_number = 1:recon_total %reconstruct and classify and vote cycle from frame_start to frame_end
            recon_start = (recon_number-1)*100+1;
            recon_end = recon_number*100;
            x = downSample(y(recon_start:recon_end),sample_level); %reconstruct the touch signal as x
            %imagesc(reshape(x,10,10)',[-1,1]);
            classification = SRCAccuracy(x, lib_new, true_label)-1; %calculate SRC classification result label
